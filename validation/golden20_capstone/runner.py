...
PROTO = Path(
    os.environ.get("PROTOTYPE_ROOT", r"C:\Users\kbwdesktop\Desktop\dev-practice\podo-algorithm-test")
)
...
def persona_inputs(persona: str):
    resume_text = (PROTO / "data" / "eval" / "resumes" / f"{persona}.md").read_text(encoding="utf-8")
    jp = json.loads((PROTO / "outputs" / "eval" / persona / "jobs_parsed.json").read_text(encoding="utf-8"))
    items = jp if isinstance(jp, list) else jp.get("jobs", jp)
    jobs = [{"job_id": j["job_id"], "company": j.get("company", ""), "title": j.get("title", ""),
             "url": j.get("url", ""), "raw_text": j.get("raw_text", "")} for j in items]
    fr = json.loads((PROTO / "outputs" / "eval" / persona / "final_ranking.json").read_text(encoding="utf-8"))
    up = fr.get("user_profile") or {}
    return resume_text, jobs, up.get("primary_domains") or [], up.get("secondary_domains") or []
...
def main() -> None:
    ...
    gp = json.loads((PROTO / "data" / "eval" / "golden_pairs" / "golden_pairs_20.json").read_text(encoding="utf-8"))
    raw_pairs = gp["pairs"]
    ...
    log(f"personas: { {k: len(v) for k, v in by_persona.items()} } | total pairs={len(raw_pairs)}")
    log("metric: podo-ai eval.golden_pairs.evaluate_pairs (strict A/B by rank, tie-aware by fit)")
    log("")
    ...
    for persona, rows in by_persona.items():
        ...
        flush()
        resume = Resume(raw_text=resume_text, primary_domains=prim, secondary_domains=sec)
        out = run_scoring(resume, jobs, ranking_mode="domain_fit_bt")  # checkpoint 2: LIVE
        ...
        st = sum(1 for e in evald if e.pair.label in ("A_better", "B_better") and not e.unavailable)
        ...
        for r in sorted(rk, key=lambda r: r["rank"]):
            log(f"     #{r['rank']} {r['job_id']:24} fit={r['fit_level']} dom={r['domain_alignment']}")
        for e in evald:
            detail.append({
                "persona": persona, "pair_id": e.pair.pair_id,
                "job_a": e.pair.job_a, "job_b": e.pair.job_b,
                "label": e.pair.label, "system_winner": e.system_winner,
                "rank_a": ranking_map.get(e.pair.job_a), "rank_b": ranking_map.get(e.pair.job_b),
                "fit_a": fit_by_id.get(e.pair.job_a), "fit_b": fit_by_id.get(e.pair.job_b),
                "correct_strict": e.correct_strict, "correct_tie_aware": e.correct_tie_aware,
                "unavailable": e.unavailable, "category": e.pair.category,
            })
        log("")
        flush()

    strict_correct = sum(1 for e in all_eval if e.correct_strict is True)
    strict_total = sum(1 for e in all_eval if e.pair.label in ("A_better", "B_better") and not e.unavailable)
    unavailable = sum(1 for e in all_eval if e.unavailable)
    tie_correct = sum(1 for e in all_eval if e.correct_tie_aware is True)
    tie_total = sum(1 for e in all_eval if e.pair.label in ("A_better", "B_better", "tie") and not e.unavailable)
    ...
    log(f"  STRICT (rank-based, A_better/B_better): {strict_correct}/{strict_total}  acc={acc:.2f}")
    log(f"  tie-aware: {tie_correct}/{tie_total}")
    log(f"  unavailable pairs (excluded): {unavailable}  | total scored: {strict_total}/20")
    log(f"  prototype baseline: 16/20 (acc=0.80)")
    log("")
    log("DISAGREEMENTS (system != human, strict):")
    for e in all_eval:
        if e.pair.label in ("A_better", "B_better") and e.correct_strict is False and not e.unavailable:
            log(f"  {e.pair.pair_id}: label={e.pair.label} system={e.system_winner} "
