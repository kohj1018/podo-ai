"""T-074 오늘의집(Bucketplace) careers 어댑터 (Tier3 custom).

bucketplace.com/careers JSON API → RawJob list.
...
_BUCKETPLACE_API = "https://bucketplace.com/api/careers/jobs"
_REQUIRED_FIELDS = ("id", "title", "url")


class BucketplaceAdapter(BaseCustomAdapter):
    """오늘의집(Bucketplace) careers 어댑터."""

    _required_fields = _REQUIRED_FIELDS

    def __init__(
        self,
        company: str = "bucketplace",
        *,
        client: Any | None = None,
        base_url: str | None = None,
    ) -> None:
        super().__init__(
            company=company, client=client, base_url=base_url or _BUCKETPLACE_API
        )
