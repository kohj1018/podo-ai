"""T-075 현대 그룹 통합 talent 포털 어댑터 (talent.hyundai.com).

현대자동차 본사. 계열사(오토에버·모비스·기아)는 candidate 상태(확정 필요).
...
_HYUNDAI_API = "https://talent.hyundai.com/api/jobs"
_REQUIRED_FIELDS = ("id", "title", "url")


class HyundaiAdapter(BaseCustomAdapter):
    """현대 talent 포털 어댑터."""
    ...
    def __init__(
        self,
        company: str = "hyundai-motor",
        *,
        client: Any | None = None,
        base_url: str | None = None,
    ) -> None:
        super().__init__(
            company=company, client=client, base_url=base_url or _HYUNDAI_API
        )
