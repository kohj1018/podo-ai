...
## 3. 구현 항목
- `ai/worker/src/worker/pipeline.py`(또는 `worker/__main__.py`) — `run_scoring(resume, jobs, ranking_mode="domain_fit_bt")` 오케스트레이션 + 후보 집합 헬퍼.
- `ai/worker/src/worker/report.py` — FitResult/coverage 필드 → JSONB 계약 직렬화(SPEC §12). 합격확률/% 출력 금지 + BT="상대 강도" 명시 보존.
...
## 4-1. 변경 예정 파일/경로
- `ai/worker/src/worker/pipeline.py`, `ai/worker/src/worker/report.py`, `ai/worker/tests/test_pipeline.py`
...
- AC-5 [Given] (Resume, 원본 jobs) [When] `run_scoring(resume, jobs)` [Then] 단계 1~12를 단일 진입점에서 실행하고 `compute_fit`을 공고당 1회만 호출(다운스트림 7·8·11 재계산 0)하며 AC-2 산출물 계약을 만든다.
...
- AC-5 → pytest::ai/worker/tests/test_pipeline.py::test_AC_5_full_entry_runs_1_to_12_compute_fit_once
...
## 8. 메모
이 task가 F-001·F-003을 통합하는 진입점. eval 하니스(T-014/T-016)가 본 산출물을 읽는다. AC 5개는 통합 task(6 deps 묶음)라 정당 — 추가 분해보다 진입점 1개 유지가 정합(SPEC §2 단계 시퀀스 단일 소유).
- repair-workitem 2026-06-05 P0 F401: Adopt-modified — DOM_RANK 중복 제거+import 사용, FitResult 삭제, LLMCallError는 진입점/test에서 사용
- repair-workitem 2026-06-05 P1 scope: Adopt — run_scoring(Resume, jobs) 1~12 단일 진입점 복원(+AC-5), 7~12는 _rank 분리, compute_fit 공고당 1회 공유

## 9. 의존성
- depends_on: [T-003, T-006, T-007, T-008, T-009, T-010]
- read_set: ["ai/worker/**", "ai/core/src/core/models.py"]
- write_set: ["ai/worker/src/worker/pipeline.py", "ai/worker/src/worker/report.py", "ai/worker/tests/test_pipeline.py"]
