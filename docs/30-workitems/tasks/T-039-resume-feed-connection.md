...
## 4-1. 변경 예정 파일/경로
- `podo/apps/web/components/ResumeUpload.tsx` — "분석 시작" onClick: `POST /api/v1/resumes/:id/score`(T-037) → localStorage(`podo_active_resume_id`) → feed(`/`) 이동(`onNavigateFeed` override / 기본 `window.location.assign`). preview state에 `resumeId` 추가. (AC-1)
- `podo/apps/web/test/resume_feed.spec.tsx` (신규, §6-1) — AC-1: 업로드→분석시작→score 호출→nav→FeedList 적합도 배지(level 4) 단언
- `podo/apps/web/components/FeedList.tsx` — 미변경(write_set이나 §3 item2 "단일 사용자 M3=최신 ranking_run 충분"으로 기존 렌더 재사용)
...
- repair-workitem 2026-06-07 P1 AC1-nav-unwired: Adopt-modified — page.tsx no-op gap 해소. useRouter는 router context 부재로 타 spec 깨므로 ResumeUpload 기본 nav=`window.location.assign('/')`, `onNavigateFeed` override 유지(테스트 보존).
