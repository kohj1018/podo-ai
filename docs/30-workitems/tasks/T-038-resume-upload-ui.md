...
## 4-1. 변경 예정 파일/경로
- `podo/apps/web/app/resume/page.tsx` (신규) — `/resume` 라우트 셸 + ResumeUpload 렌더 (AC-1)
- `podo/apps/web/components/ResumeUpload.tsx` (신규) — 파일/paste 업로드 → `POST /api/v1/resumes` → preview 상태 + "분석 시작" 버튼(preview 전 disabled) (AC-1, AC-3)
- `podo/apps/web/components/MaskingPreview.tsx` (신규) — 마스킹본 렌더 + `[MASKED_*]` 토큰색 강조(band-3) + evidence 요약 (AC-2)
- `podo/apps/web/test/resume_upload.spec.tsx` (신규, §6-1) — AC-1/2/3 테스트
- `docs/20-system/DESIGN.md` §7-2 — ResumeUpload·MaskingPreview 인벤토리 등록 (step5, P1-design)
- `podo/apps/web/app/globals.css` (**write_set 이탈** — read_set이었으나 필요 enabling) — `--grape-600/700/100`·`--surface` 토큰 추가. DESIGN §2-1과 정확 동기(grape-600 #8B3DD4·grape-700 #6F2BC0·grape-100 #F1E6FB·on-accent #FFFFFF); Button.primary accent=grape-600 렌더에 필수이나 직전 globals.css에 부재했던 토큰. revert 아닌 동기 유지.
...
- repair-workitem 2026-06-07 P0 lint-noArrayIndexKey: Adopt — MaskingPreview `key={i}`→누적 오프셋 key(`${offset}-${part}`); `pnpm validate` biome 차단 해소.
- repair-workitem 2026-06-07 P1 Design-inventory: Adopt — DESIGN §7-2에 ResumeUpload·MaskingPreview 등록(8-state 범위 주석); AC-1 충족.
- repair-workitem 2026-06-07 P1 token-rawhex: Adopt — ResumeUpload 버튼 `var(--grape-600,#hex)`→`var(--grape-600)`, `--surface` 동일; DESIGN §2 raw hex 0.
- repair-workitem 2026-06-07 P1 scope-undocumented: Adopt — globals.css grape/surface는 DESIGN §2-1 정확 동기·필수 enabling이라 유지, §4-1에 write_set 이탈 명시(revert 아님).
