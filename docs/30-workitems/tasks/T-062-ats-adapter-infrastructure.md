...
## 2. 작업 범위
- `crawler/src/crawler/adapters/base.py` — ATS 어댑터 공통 인터페이스(`BaseCrawlerAdapter`: `fetch_jobs(location: str = "KR") -> list[JobPosting]`, `gate_check() -> GateResult`). **location 1급 파라미터**(외국계 한국 채용 필터 — 구체 적용은 T-071 ATS·T-072 custom).
...
- `crawler/src/crawler/sources/` 기존 toss/karrot 어댑터: `BaseCrawlerAdapter` 상속으로 마이그레이션(동작 불변 — 리팩토링 범위 명시, ADR-101#amend).
- 오프라인 fixture 테스트(`crawler/tests/fixtures/greenhouse_*.json`) — 무키 E2E 보존(M2 패턴).

## 3. 구현 항목
1. `crawler/src/crawler/adapters/base.py` — 신설. `BaseCrawlerAdapter(ABC)`: `fetch_jobs(location: str = "KR") -> list[JobPosting]`, `gate_check() -> GateResult(ok: bool, reason: str)`. `GateResult` dataclass 정의. location은 어댑터별(ATS/custom) 구현 — base는 시그니처만. → 확인: mypy 통과 (AC-1)
2. `crawler/src/crawler/adapters/greenhouse.py` — 신설. `GreenhouseAdapter(BaseCrawlerAdapter)`: `company_slug` + `base_url=https://boards-api.greenhouse.io/v1/boards/{slug}/jobs` httpx GET, 페이지네이션(`offset` 파라미터), `JobPosting` 변환. `gate_check()`: 2xx 이외 or parse 실패율 ≥30% → `ok=False`. → 확인: fixture 기반 단위 테스트 pass (AC-1)
...
4. `crawler/src/crawler/sources/registry.py` — 신설. `SourceRegistry`: `sources: list[SourceEntry(company, adapter_cls, tier, status)]`. `register(entry)`, `get_active_sources() -> list[SourceEntry]`. 초기 등록: toss(bespoke), karrot(bespoke), greenhouse 사용 회사 ≥1개(예: Moloco). → 확인: registry 조회 테스트 (AC-1)
5. `crawler/src/crawler/sources/toss.py`, `crawler/src/crawler/sources/karrot.py` — 기존 bespoke 로직 유지, `BaseCrawlerAdapter` 상속으로 시그니처 맞춤(외부 행동 불변). → 확인: 기존 tests/crawler 통과 (AC-1, 회귀 없음)
...
- `crawler/src/crawler/sources/toss.py` (BaseCrawlerAdapter 상속 추가)
- `crawler/src/crawler/sources/karrot.py` (BaseCrawlerAdapter 상속 추가)
...
- AC-1 [Given] Greenhouse fixture JSON [When] `GreenhouseAdapter.fetch_jobs()` 호출 [Then] `JobPosting` list를 반환하고 소스 레지스트리에 tier·status 포함 등록되며, toss/karrot 기존 어댑터가 동일 인터페이스로 동작한다.
...
## 8. 메모
- `BaseCrawlerAdapter` 공통 인터페이스는 3회 이상 반복 패턴(toss·karrot·greenhouse)이 확인된 시점에 도입 — YAGNI 요건 충족(ADR-006).
...
- assumptions: ["JobPosting 모델은 `ai/core`(crawler가 `from core.models` import) — builder 확인", "기존 toss/karrot 크롤러 crawler/tests/* 통과 상태"]
