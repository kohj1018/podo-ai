...
## 2. 작업 범위
- `crawler/src/crawler/adapters/base.py` — ATS 어댑터 공통 인터페이스(`BaseCrawlerAdapter`: `fetch_jobs() -> list[JobPosting]`, `gate_check() -> GateResult`).
...
## 3. 구현 항목
1. `crawler/src/crawler/adapters/base.py` — 신설. `BaseCrawlerAdapter(ABC)`: `fetch_jobs() -> list[JobPosting]`, `gate_check() -> GateResult(ok: bool, reason: str)`. `GateResult` dataclass 정의. → 확인: mypy 통과 (AC-1)
...
- 열린 질문: Greenhouse `boards-api` 엔드포인트는 인증 불필요(public). Lever/Workday는 T-063에서 각 API 요건 확인 후 동일 패턴 적용.
