...
## 4-1. 변경 예정 파일/경로
- `infra/docker-compose.yml` — LocalStack 서비스 추가
- `infra/localstack-init/01-create-queue.sh` (신규)
- `podo/apps/api/prisma/schema.prisma` — scoring_jobs 모델 추가
- `podo/apps/api/prisma/migrations/YYYYMMDD_add_scoring_jobs/migration.sql` (신규)
- `podo/apps/api/src/queue/queue.service.ts` (신규)
- `podo/apps/api/src/queue/queue.module.ts` (신규)
- `podo/apps/api/src/resumes/resumes.controller.ts` — spawn 제거 + enqueue
- `podo/apps/api/src/scoring-jobs/scoring-jobs.controller.ts` (신규)
- `podo/apps/api/src/app.module.ts` — QueueModule 등록
- `podo/apps/api/test/queue.spec.ts` (신규)
- `podo/apps/api/package.json` + `pnpm-lock.yaml`
