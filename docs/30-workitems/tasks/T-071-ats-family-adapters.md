...
## 3. 구현 항목
1. `adapters/greeting.py` — `GreetingAdapter`: `{company}.career.greetinghr.com` 목록 API/HTML, 페이지네이션, `JobPosting` 변환(한국 스타트업 1위 — 우선). → fixture 테스트 (AC-1)
...
## 4-1. 변경 예정 파일/경로
- `crawler/src/crawler/adapters/lever.py` · `ashby.py` · `workday.py` (신설)
- `crawler/src/crawler/adapters/base.py` (location 파라미터 — T-062와 정합)
- `crawler/tests/fixtures/{lever,ashby,workday}_jobs.json` (신설)
- `crawler/tests/test_ats_family.py` (신설)

## 5. 완료 조건
Lever·Ashby 어댑터가 fixture 기반으로 동작하고(Workday는 discovery에 있으면 구현/없으면 unsupported), 모든 ATS 어댑터가 location="KR" 필터로 한국 공고만 거른다.

## 6. Acceptance Criteria
- AC-1 [Given] 그리팅·Workday·Lever·Ashby fixture JSON [When] 각 어댑터 `fetch_jobs()` [Then] `JobPosting` list 반환 + 페이지네이션 처리 + 기존 `BaseCrawlerAdapter` 인터페이스 정합(우선순위 그리팅·Workday).
