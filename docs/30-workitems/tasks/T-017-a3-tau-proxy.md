...
## 3. 구현 항목
- `ai/eval/a3_tau.py` — τ 계산(pairwise 라벨 → Kendall τ) + 자명 페어 위반율 + 판정 리포트.
...
## 4-1. 변경 예정 파일/경로
- `ai/eval/a3_tau.py`, `ai/eval/tests/test_a3_tau.py`
...
## 9. 의존성
- depends_on: [T-016]
- read_set: ["ai/eval/golden_pairs.py", "ai/worker/**"]
- write_set: ["ai/eval/a3_tau.py", "ai/eval/tests/test_a3_tau.py"]
