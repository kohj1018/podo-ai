...
## 4-1. 변경 예정 파일/경로
- `ai/tests/fixtures/pii_resume.txt` (신규) — 알려진 raw PII(이름·email·전화·주민번호·개인URL) + evidence(스택/학교/기간) fixture
- `ai/tests/test_pii_safety.py` (신규) — AC-1: fixture sanity → 마스킹 → 채점(keyless fake) → 6표면 literal scan = 0

## 4-2. 구현 메모
- 마스킹은 known-value 오라클(고정 fixture의 알려진 PII만 placeholder 치환 — ADR-105 정합). 마스커 *패턴* 정확성은 T-036(TS 단위테스트), 실 NestJS 마스커→DB end-to-end는 stabilize E2E(실 업로드)가 실증. 본 게이트의 핵심 가치 = *하류 표면*(ranking_runs.result·recommendations·cache·log = F-014 §7 주 누출 표면)이 채점 후 raw PII 0 + 커밋 웜캐시 무오염.
- 채점은 fake call_fn 주입(무키, M2 패턴) — .cache/llm 신규 write 없음. 트랜잭션 rollback으로 DB 멱등.
