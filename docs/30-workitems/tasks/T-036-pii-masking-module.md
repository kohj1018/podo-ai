...
## 4-1. 변경 예정 파일/경로
- `podo/apps/api/src/resumes/resume-masker.port.ts` — `RegexResumeMasker` 전체 구현 추가(RRN·email·phone·URL[기술프로필 보존]·이름 레이블 best-effort). stub은 호환 위해 export 유지.
- `podo/apps/api/src/resumes/resumes.module.ts` — useClass: RegexResumeMaskerStub → RegexResumeMasker.
- `podo/apps/api/test/masker.spec.ts` (신규) — AC-1 직접식별자 마스킹 + AC-2 evidence 보존(순수함수).
- `docs/90-decisions/project/ADR-105-pii-masking-policy.md` (신규) — PII 마스킹 정책(status=accepted).
- `docs/20-system/ARCHITECTURE_OVERVIEW.md` — §8 보안 + §10 열린질문에 ADR-105 backref(AC-3).
...
- 구현 메모(2026-06-07): 구현 중 builder가 `class-validator`/`class-transformer`를 package.json/lock에 추가(미사용·범위 밖, §9 write_set 미포함) → 메인 세션에서 `git checkout`으로 되돌림(YAGNI/ADR-006). 마스킹은 순수 regex라 신규 의존성 0.
