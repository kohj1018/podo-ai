...
## 4-1. 변경 예정 파일/경로
- `podo/apps/web/components/ResumeUpload.tsx` — 비허용 포맷/크기 검증, 로딩 skeleton(.shimmer 클래스), error toast 추가
- `podo/apps/web/test/resume_states.spec.tsx` — AC-1, AC-2 테스트 신규 생성 (+ skeleton .shimmer hook 단언)
- `podo/apps/web/app/globals.css` (**write_set 이탈** — read_set·write_set 미포함이나 필수 enabling) — DESIGN §2 토큰 `--line`/`--line-strong`/`--muted` 동기(직전 globals.css 미존재) + `.shimmer` `@keyframes` + `@media (prefers-reduced-motion: reduce)` 분기. AC-2의 reduced-motion 절은 React inline style로 표현 불가 → CSS 규칙 필수(T-038 globals.css 선례 동형).
...
- repair-workitem 2026-06-07 P1 AC2-undefined-token: Adopt — globals.css에 DESIGN §2 `--line`/`--line-strong`/`--muted` 동기; skeleton 색 실렌더(미정의 var → 보임).
- repair-workitem 2026-06-07 P1 AC2-shimmer-reduced-motion: Adopt — globals.css에 `.shimmer` keyframes + `@media (prefers-reduced-motion: reduce)` 추가(inline 불가); AC-2 절 충족. skeleton inline `background:var(--line)` 제거(클래스 gradient 노출).
