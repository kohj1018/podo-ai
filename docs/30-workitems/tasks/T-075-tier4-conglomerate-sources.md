...
- **로그인 처리**: T-070이 회사별 *목록 view 로그인 여부* 확정 → list-public만 수집, list-login은 status=login-required(미수집·투명). 그룹 통합포털은 계열사 필터 필요.
...
## 4-1. 변경 예정 파일/경로
- `crawler/src/crawler/sources/registry_seed.py` (Tier4 등록 + view_login status)
