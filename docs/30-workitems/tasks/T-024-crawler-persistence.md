...
- 구현 노트(2026-06-06): 메인 세션 수동(M2 포크 사망 패턴). persistence.py는 source별 url 기준 diff(신규/유지/마감). **마감임박 N/A** — fetch(fetch_jobs)가 closing_at 미산출(RawJob={job_id,company,title,url,raw_text}). closing_at 수집은 후속.
- role_family는 upsert 시 `selection.classify_role_family(title)`로 enrich(job_postings.role_family — crawler 소유 분류).
- 단건 skip: fetch_jobs의 *상세 fetch*만 try/except httpx.HTTPStatusError. *목록 fetch* 실패는 source 전체 실패로 collect/CoverageState 처리(store.py 기존). status='partial'은 오케스트레이션(T-025)이 failure 유무로 분류.
- 라이브 검증: DATABASE_URL 주입 시 2 테스트(AC-1 멱등/diff, AC-2 crawl_run/단건skip) green.
