...
## 4-1. 변경 예정 파일/경로
- `ai/worker/src/worker/persistence.py`, `ai/worker/tests/test_persistence.py`
- (`cache.py`는 무수정 — §8 참조: cache 테이블 부재 + 디스크 캐시 사용 → Postgres CacheAdapter YAGNI, make_key 불변)
...
- 구현 노트(2026-06-06): implement 포크 조기 사망(파일 미생성) → 메인 세션 수동. result JSONB는 기존 `report.build_report`(SPEC §12 계약 직렬화기) 호출 결과를 저장 — persist_run은 파싱/재계산 안 함(verbatim).
- §3-3 cache.py **무수정**: 스키마에 cache 테이블 없음(T-020 5테이블) + E2E가 디스크 캐시(.cache/llm) 사용 → Postgres CacheAdapter 소비자 0 = YAGNI(ADR-006). AC-2 요구("make_key 바이트 동일")는 cache.py 무수정으로 충족(make_key 불변). 해석: AC-2의 검증 가능 계약은 make_key 불변이며 새 어댑터는 비범위.
- held 순서 결정성: persist_run이 `sorted(pending, key=int)`로 rank_position 부여(결정적). `pending_job_ids`가 set이라 build_report `list(set)`은 프로세스 간 비결정 가능(report.py:52 사전 부채, T-022 범위 밖 — stabilize 후보).
- persist_run 시그니처: `(conn, resume_id, jobs, result, *, ranking_mode)`. 복합키: model=config.OPENAI_MODEL·prompt_version=config.PROMPT_VERSION·scoring_mode/ranking_mode/cache_key_version=상수. 라이브 검증: DATABASE_URL 주입 시 3 테스트 green.
