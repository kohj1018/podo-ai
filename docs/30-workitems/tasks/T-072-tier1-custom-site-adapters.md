...
## 2. 작업 범위
- `crawler/src/crawler/adapters/custom_base.py` — `BaseCustomAdapter(BaseCrawlerAdapter)`: 공통 골격(**httpx + BeautifulSoup4(현 deps)** — 내부 JSON API 발견 우선·실패 시 HTML 파싱, 페이지네이션 추상, parse 실패율 게이트, location 필터). **Playwright는 현 의존성 아님**(추가는 별도 결정).
...
- 동적 렌더 소스: **우선 내부 API 탐색(httpx 직접)**. 그래도 JS 필수면 Playwright 의존성 추가(명시 결정) 또는 status=`unsupported`(후속) — 내부API/정적 우선.
...
2. Tier1 custom 어댑터(discovery custom-ready 회사) — 회사별 `XxxAdapter(BaseCustomAdapter)`: careers endpoint, 파싱 매핑, RawJob(dict) 변환. 본사 우선, 계열사는 가능 시 config 재사용. → fixture 테스트 (AC-1, AC-2)
3. 동적 렌더 소스 → 내부 API 우선; JS 필수 시 Playwright 의존성 추가(결정) 또는 unsupported. → 해당 fixture (AC-1)
...
- AC-1 [Given] Tier1 custom 회사 fixture(내부 API 또는 HTML 스냅샷) [When] 해당 `XxxAdapter.fetch_jobs(location="KR")` [Then] RawJob(dict) list 반환 + `BaseCustomAdapter` 골격 재사용 + 한국 공고 필터.
...
- 자체사이트는 구조변경 취약 → A-1 게이트 필수(각 어댑터). Playwright는 현 deps 아님 → JS 불가피 소스만 의존성 추가 결정, 아니면 내부 API/unsupported.
