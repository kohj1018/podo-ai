...
## 1. 작업 목적
F-021 비용 구조 전환(N→K) 전/후 LLM 비용(토큰·호출 수)을 실측 비교해 절감을 입증한다(**M5 필수**). **A-3 τ(상대 랭킹 타당도)는 선택/후속 게이트**(M5 §5·결정 #11) — 창업자 수기 랭킹 라벨 준비 시 확대 실데이터로 1회 측정·판정, 미준비 시 후속(졸업 비차단). 하니스(`ai/eval/a3_tau.py`)는 M1(T-017) 기구현. 비용 비교는 F-021 출력 계약 불변 보존 확인도 겸한다(F-023 §3 Alternate path).
...
- `ai/eval/src/eval/run_a3_m5.py` (신설 — plan §3 경로 `ai/eval/run_a3_m5.py`에서 패키지 경로로 변경, import 계약 충족 필요. validation report Diff trace audit P2 기록)
