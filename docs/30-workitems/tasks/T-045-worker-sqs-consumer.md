...
## 4-1. 변경 예정 파일/경로
- `ai/worker/src/worker/__main__.py` — main()을 SQS long-poll consumer로 전환(process_message/consume_once/consume_loop + 상태 신호 emit + 재시도/backoff). run()/seed `--resume-id` 경로 보존.
- `ai/worker/pyproject.toml` — boto3 의존성 추가 / `uv.lock`
- `pyproject.toml` — mypy boto3/botocore ignore_missing_imports override(stub 부재)
- `infra/localstack-init/01-create-queue.sh` — scoring-status-queue 추가 생성(상태 신호 큐)
- `podo/apps/api/src/scoring-jobs/scoring-jobs.controller.ts` — ranking_run join으로 done 판정
- `podo/apps/api/test/queue.spec.ts` — scoring-jobs 폴링 fakePrisma에 rankingRun.findFirst 추가 + done-via-join 테스트(T-045 join ripple)
- `ai/tests/test_worker_consumer.py` (신규 — AC-1/2/3 DB, AC-4 fake conn)
- (비채택) docker-compose worker 컨테이너 — 아래 메모 참조(M4는 worker 호스트 실행)
...
- **구현 결정(implement) — 상태 신호 채널 추가**: §8의 "join-only(ranking_run 존재=done)"는 AC-4(failed)를 표현 못 함 → worker가 `scoring-status-queue`로 running/done/failed **이벤트 emit**(§3 item4)을 채택. api는 §4-1대로 **done은 ranking_run join**으로 판정(폴링 가능). worker는 scoring_jobs 직접 write 안 함(ARCH §3-2). running/failed 이벤트의 api 반영(scoring_jobs.status 갱신 consumer)은 후속 — M4 happy-path E2E(T-052)는 done join으로 충분, failed 가시화는 후속/M6.
- **구현 결정(implement) — worker 컨테이너 비채택**: milestone §5 graduation은 `docker compose up`을 *Postgres+pgvector + LocalStack SQS*로 명시(worker 컨테이너 미포함). worker Dockerfile은 M6 소관 → M4는 worker를 **호스트 프로세스**로 실행(api/web와 동일, M2/M3 패턴). T-052 e2e.mjs가 `uv run python -m worker`를 백그라운드 호스트 프로세스로 기동(env: SQS_ENDPOINT_URL/SQS_QUEUE_URL/SQS_STATUS_QUEUE_URL/DATABASE_URL). 따라서 docker-compose.yml에 worker 서비스 미추가(§3 item3 대비 의도적 비채택).
- 검증(implement): `pytest ai/tests/test_worker_consumer.py` 4 pass(podo_test) / AC-4 무DB pass · 기존 worker pytest green · `pnpm validate` green · api vitest 32 pass(podo_test). 실 LocalStack 왕복은 T-052(E2E)가 실증.
