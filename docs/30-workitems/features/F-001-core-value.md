...
## 1. 요약
수집된 공고와 사용자 이력서에 대해, *동일 입력이면 항상 동일한* fit 점수·적합도 5단계를 산출하고, 각 점수의 근거를 JD 원문 인용으로 표시한다. 제품의 단일 thesis("틀린 점수 > 근거 없는 점수")를 직접 구현하는 핵심 feature이며, GS-1·GS-2 게이트가 이 feature 위에서 측정된다. (ADR-100 D1·D3)
...
4. fit 점수·적합도 5단계·근거(JD 인용 + 이력서↔JD 매핑)를 저장하고 반환.
