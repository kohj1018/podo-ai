...
> **#3 E2E 진행 (Fix round 2, 2026-06-06):** machinery 완료 — 결정적 crawl fixture + `scripts/e2e.mjs` 단일 오케스트레이션(`pnpm e2e`) + CI 게이트 `e2e-smoke.yml` + README runbook. 잔여 1-step: 키 보유자 `pnpm e2e:warm` → `ai/worker/fixtures/llm_cache` 커밋 → CI green 시 #3 체크·졸업 재평가. 상세 [IMPROVEMENT_GUIDE M2-E2E-001](../../40-validation/IMPROVEMENT_GUIDE.md).
