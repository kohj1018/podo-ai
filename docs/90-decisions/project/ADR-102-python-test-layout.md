...
### D5. 구현 파일은 src-layout `ai/<pkg>/src/<pkg>/`
- T-001 scaffold가 src-layout(`packages=["src/<pkg>"]`)이므로 import 가능한 구현 모듈은 `ai/<pkg>/src/<pkg>/X.py`에 둔다(`from <pkg>.X import` resolve). task 문서가 `ai/<pkg>/X.py`로 *약기*해도 실제 위치는 src-layout 경로다(검증 시 정합으로 간주).
