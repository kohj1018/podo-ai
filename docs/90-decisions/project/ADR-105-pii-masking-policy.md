...
<a id="adr-105-amend-2"></a>
## Amendment 2 — 간접 재식별 경계 확정 (공개 배포, M6 / T-089)

**배경:** Amend1이 간접 재식별 방어 시점을 M6(공개 배포)로 정정했다. T-089가 공개 배포 baseline에서 그 경계를 확정한다(원 §6 닫음).

**결정 (경계):**
1. **직접 식별자(원 §1 1~5번)는 마스킹 유지** — 이름·이메일·전화·주소·생년월일 등.
2. **학교+회사+기간 조합(간접 지표)은 evidence로 *보존* — 추가 마스킹/익명화하지 않는다.** 근거: 과마스킹은 grounding/GS-2(근거 기반 채점)를 약화시킨다(학교·회사·기간은 적합도 근거의 핵심). evidence 보존 > 간접 재식별 완전 차단.
3. **잔여 간접 재식별 리스크 = accepted-risk** — 보상 통제로 완화: ① PII at-rest 암호화(RDS `storage_encrypted`, T-089) ② raw PII 미영속(마스킹본만, 원 결정 불변) + 계정 PII 스코어링 경로 미유입(Amend1 §2) ③ 사용자 간 데이터 격리(M4) + 세션 접근통제.
4. **재평가 트리거:** 데이터 가치·규모·법적 요구 상승 시 학교+회사+기간 조합의 부분 익명화(예: 회사명 일반화) 재검토.

**적용:** 코드 변경 없음 — 현재 마스커가 이미 이 경계(직접만 마스킹, 간접 보존)와 일치한다. 본 amendment는 그 경계를 *의식적 결정*으로 명문화한다(무의식적 누락 아님). PII 불변식은 `scripts/e2e_pii_scan.py`·`e2e_account_pii_scan.py`가 배포 환경에서 재검증.
