...
>
> **Fix round 2 (E2E) 2026-06-06** (메인 세션, 사용자 결정=웜캐시 커밋): **M2-E2E-001 (b)(c) machinery 완료** — 결정적 crawl fixture(`crawler/fixtures/seed_jobs.txt` + `CRAWL_FIXTURE` 오프라인 모드, `load_fixture`/`crawl(fixture_jobs=)`), 단일 오케스트레이션 `scripts/e2e.mjs`(compose up→migrate→crawl→score(웜캐시)→api 서빙→feed/coverage assert) + `pnpm e2e`/`e2e:warm`, CI 졸업 게이트 `.github/workflows/e2e-smoke.yml`(무키), 웜캐시 tracked dir `ai/worker/fixtures/llm_cache`(`LLM_CACHE_DIR`로 `.cache/llm`과 분리), README/README_ko E2E runbook([Doc-stale] resolved). **잔여 1-step**: 키 보유자가 `pnpm e2e:warm` 1회 → `ai/worker/fixtures/llm_cache/*.json` 커밋. 그 커밋 + CI green이 graduation §5 #3을 닫는다.

- **M2-E2E-001** | P0 | [관측됨] | linked: M2,T-018,T-022,T-023 | status: machinery 완료, 웜캐시 커밋 1-step 대기(Fix round 2) | `crawler/__main__.py` · `scripts/e2e.mjs` · `.github/workflows/e2e-smoke.yml` · `ai/worker/fixtures/llm_cache/`
  ...
  - 해소: (a)(d) 커밋 `57df9fc`. (b) 웜캐시 경로 확정 — fixture 공고로 캐시 키 고정(`make_key`는 JD raw_text 기반이라 DB id 무관) + `LLM_CACHE_DIR` tracked dir. (c) `scripts/e2e.mjs` + `pnpm e2e` + CI 게이트 + README runbook.
  - 잔여: 웜캐시는 실 LLM 1회 실행(`pnpm e2e:warm`, OPENAI_API_KEY)로만 생성 가능 → 키 보유자가 생성·커밋. **그 전까지 `pnpm e2e`·e2e-smoke는 "웜캐시 비어있음" 명시 에러로 실패(의도된 게이트)**.
  - 권장: 키 보유자 `pnpm e2e:warm` → `git add ai/worker/fixtures/llm_cache` 커밋 → CI e2e-smoke green 확인 → milestone §5 #3 체크 → graduation 재평가(YES).
