...
### 디자인 원칙 (5)
1. **따뜻한 chrome, 엄격한 data** — 마스코트·카피·형태·모션이 친근함을 전담한다. 점수·합격가능성 밴드·근거는 장식 없이 정확·정직하게. 귀여움이 *판단 데이터*를 장식하지 않는다.
2. **포도 보라는 brand 신호로만** — 포도 그라데이션은 로고·fit 링·인사 strip 등 *정해진 brand 순간*에만 contained. 전면/카드 배경 그라데이션 금지(슬롭 차단).
3. **랭킹과 근거를 한 시선에** — fit 점수 · 5단계 합격가능성 밴드 · 근거를 카드 한 장에서 즉시 위계적으로. 스캔 → 펼침 → 결정.
...
- **색만으로 의미 전달 금지**: 합격가능성 밴드·매핑은 항상 텍스트 라벨(+✓/△) 동반. band 색을 텍스트로 쓸 땐 `band-*-ink`(AA).
