...
## grounding 출처   <!-- R0 -->
- 사용자 제공 마스코트 이미지 [`podo/apps/web/public/podo-mascot.png`](../../podo/apps/web/public/podo-mascot.png) (1차 grounding — 색·형태·표정 톤의 직접 출처). 최초 `docs/20-system/assets/`에 임시 보관(모노레포 생성 전) → M4에서 web 프론트엔드 `public/`로 이관해 단일 위치로 사용(GreetingCard·온보딩·상태 마스코트, `PodoMascot` 컴포넌트).
